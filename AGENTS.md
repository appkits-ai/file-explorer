...
Load `.agents/skills/virgin-principle/SKILL.md` for every durable change, then `.agents/skills/issue-delivery/SKILL.md` for linking rules. Work from `/home/agent/workspace/appkits/file-explorer` on `main`, or from a repo-local `.worktrees/<task>` checkout created by `git worktree`.
